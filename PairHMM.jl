  ...
  function mcmc_sequencepair(citer::Int, niter::Int, samplerate::Int, rng::AbstractRNG, initialSample::SequencePairSample, modelparams::ModelParameters, cornercut::Int=100, fixAlignment::Bool=false, writeoutput::Bool=false, outputdir::AbstractString="")
    ...
    moveWeights = Float64[0.25, 0.0, 40.0, 100.0, 100.0, 100.0, 100.0]
    if fixAlignment
  ...
  function train()
    ...
    useswitching = false
    useparallel = true
    pairs = shuffle!(rng, load_sequences("data/data.txt"))[1:64]
    ...
    for i=1:maxiters
      ...
      if useparallel
        refs = RemoteRef[]
        for k=1:length(pairs)
          ref = @spawn mcmc_sequencepair(currentiter, mcmciter, samplerate, MersenneTwister(abs(rand(Int))), SequencePairSample(current_samples[k]), deepcopy(modelparams), cornercut)
          push!(refs,ref)
        end
  ...
  function test()
    ...
    modelfile = "models/pairhmm8_switching.jls"
    outputdir = "logs/pairhmm8_switching/"
    ...
    for k=1:length(pairs)
      ...
      ret = mcmc_sequencepair(0, 4000, 1, rng, current_sample, modelparams, cornercut, fixAlignment, true, outputdir)
