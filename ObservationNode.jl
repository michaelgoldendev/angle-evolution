...
type ObservationNode
  ...
  function ObservationNode()
    ...
    new(aapairnode, DiffusionNode(),ss,false, SwitchingNode(),false)
  end

  function ObservationNode(node::ObservationNode)
    new(AAPairNode(node.aapairnode), DiffusionNode(node.diffusion), SecondaryStructureNode(node.ss), node.usesecondarystructure, SwitchingNode(node.switching), node.useswitching)
  end
end
